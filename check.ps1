...
$configPath = Write-CheckText -Ctx $ctx -RelativePath 'inputs/bloom.json' -Content @'
{
  "expected_items": 100,
...
$tool = Join-Path $ctx.OutputsDir 'bloom-dedup-demo.exe'
Invoke-CheckCommand -Ctx $ctx -Name 'build_cli' -Command "& '$($ctx.GoCmd)' build -o '$tool' ./cmd/bloom-dedup-demo"

$generated = Join-Path $ctx.InputsDir 'generated_ids.jsonl'
$jsonResult = Join-Path $ctx.OutputsDir 'result.json'
$mdReport = Join-Path $ctx.OutputsDir 'report.md'
$benchLog = Join-Path $ctx.OutputsDir 'bench_output.txt'

Invoke-CheckCommand -Ctx $ctx -Name 'cli_generate' -Command "& '$tool' generate --count 50 --duplicate-ratio 0.20 --out '$generated' --seed 42 --sources 3"
Invoke-CheckCommand -Ctx $ctx -Name 'cli_run_json_and_report' -Command "& '$tool' run --in '$generated' --config '$configPath' --out '$jsonResult' --report '$mdReport'"
Invoke-CheckCommand -Ctx $ctx -Name 'cli_bench' -Command "& '$tool' bench --in '$generated' --config '$configPath' | Tee-Object -FilePath '$benchLog'"

$expectedArtifacts = [ordered]@{
    generated_input = Test-Path -LiteralPath $generated
    json_result = Test-Path -LiteralPath $jsonResult
    markdown_report = Test-Path -LiteralPath $mdReport
    bench_output = Test-Path -LiteralPath $benchLog
}
Save-CheckJson -Path (Join-Path $ctx.OutputsDir 'artifact_presence.json') -Value $expectedArtifacts

Add-CommandFeatureAssessment -Ctx $ctx -Id 'minimum.generator' -Level 'minimum' -Category 'cli' -Requirement 'JSONL generator with duplicates seed and source' -CommandName 'cli_generate' -RequiredArtifacts @($generated)
Add-CommandFeatureAssessment -Ctx $ctx -Id 'minimum.bloom_run' -Level 'minimum' -Category 'algorithm' -Requirement 'Bloom Add and MayContain are exercised by run' -CommandName 'cli_run_json_and_report' -RequiredArtifacts @($jsonResult)
Add-SourceFeatureAssessment -Ctx $ctx -Id 'minimum.parameters' -Level 'minimum' -Category 'algorithm' -Requirement 'Automatic m and k calculation' -Patterns @('false_positive_rate','\b[Mm]\b|\bm\b','\b[Kk]\b|\bk\b') -Match 'all'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'minimum.exact_map' -Level 'minimum' -Category 'algorithm' -Requirement 'Exact map deduplication exists' -Patterns @('map\[string\]','exact') -Match 'all'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'minimum.false_positive' -Level 'minimum' -Category 'algorithm' -Requirement 'False positives are measured' -Patterns @('false.?positive') -Match 'any'
Add-CommandFeatureAssessment -Ctx $ctx -Id 'minimum.json_report' -Level 'minimum' -Category 'format' -Requirement 'JSON report is created by run' -CommandName 'cli_run_json_and_report' -RequiredArtifacts @($jsonResult)
...
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.counting_bloom' -Level 'excellent' -Category 'algorithm' -Requirement 'Counting Bloom filter' -Patterns @('counting_bloom|CountingBloom') -Match 'any'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.hash_variants' -Level 'excellent' -Category 'algorithm' -Requirement 'Several hash variants' -Patterns @('fnv64','sha256') -Match 'all'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.no_exact_mode' -Level 'excellent' -Category 'performance' -Requirement 'Mode without exact map' -Patterns @('exact-compare|ExactCompare') -Match 'any'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.multi_fpr_report' -Level 'excellent' -Category 'report' -Requirement 'Report for several false_positive_rate values' -Patterns @('false_positive_rate','\[\].*Report|Reports') -Match 'all'
Add-SourceFeatureAssessment -Ctx $ctx -Id 'excellent.global_vs_source' -Level 'excellent' -Category 'algorithm' -Requirement 'Global versus per-source dedup comparison' -Patterns @('per.?source|BySource','global') -Match 'all'
